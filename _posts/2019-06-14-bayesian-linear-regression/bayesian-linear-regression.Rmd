...
The parameters of the model include the intercept (or overall mean) $\alpha \in \mathbb{R}$, the coefficients of the predictor variables, $\beta \in \mathbb{R}^m$ and the standard deviation of the un-modelled noise, $\sigma \in \mathbb{R}$. 
...
sample_tau <- function(ys, alpha, beta, alpha0, beta0) {
  rgamma(1,
    ...
    rate = beta0 + 0.5 * sum((ys$y - (alpha + t(ys$x) %*% beta))^2)
  )
}

sample_alpha <- function(ys, beta, tau, mu0, tau0) {
  prec <- tau0 + tau * nrow(ys)
  mean <- (tau0 + tau * sum(ys$y - t(ys$x) %*% beta)) / prec
...
```{r gibbs-sample-lm, eval=run_mcmc}
plan(multiprocess)
