...
Unfortunately the twitteR API is a little outdated and requires tweets to be less than 140 characters (however this can be bypassed using `bypassCharLimit = TRUE`) and we can't post multiple images in the same tweet using the `mediaPath` argument. So unfortunately I had to resort to posting my graphs manually!
