...
The same bivariate Normal model from a [previous post implementing the Metropolis algorithm](2019/02/11/efficient_mcmc_rcpp) is used. See that post for details of deriving the log-likelihood and choice of prior distributions for the parameters.
...
write_rds(out_hmc, "~/git/blog/static/models/hmc_bivariate.Rds")
```

```{r, echo=FALSE}
out_hmc <- read_rds("~/git/blog/static/models/hmc_bivariate.Rds")
```

The traceplots of the chains both chains are plotted below.
...
This could be compared to a similar method, such as the [Metropolis algorithm](2019/02/11/efficient_mcmc_rcpp).
...
out_met <- metropolis(theta, log_posterior(xs), proposal, 10000, chains = 2, parallel = TRUE)
```

```{r}
out_met <- read_rds("~/git/blog/static/models/metropolis_parallel.Rds")
```
