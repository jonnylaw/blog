...
broom::augment(fit_northern)
```

Next we plot a Normal Q-Q plot to check the distribution of the residuals is Normal. If the residuals (the errors, $Y_i - \varepsilon_i, i = 1,\dots,N$) are Normally distributed, this means the conclusions drawn from the model are valid. The Q-Q plots look reasonable (except maybe the midlands (centre)) with only a few outliers identified by R.
...
The linear regression can be used as a model for the latent continuous variable $y^\star$ which is then used to fit an ordered logit model where the conditional distribution of an observed position at the national, $y$ is:

$$P(y=k|x) = P(\theta_{k-1} < y^\star \leq \theta_k|x)$$

For an arbitrary position $k$.

```{r}
library(brms)
fit <- brm(position_national ~ position_northern, data = national_northern, family = sratio(link = "probit"))
```
