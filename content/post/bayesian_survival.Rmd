...
Consider an arbitrary interval where the expected number of events in the interval is denoted as $\lambda$. The number of events in this interval is Poisson distributed with rate $\lambda$. To see this, proceed to subdivide the interval into $n$ smaller intervals $t_1, \dots, t_n$ in which the probability of an event occuring in each small interval is $\lambda / n$ and can be represented as an independent Bernoulli trial. The number of events in the entire interval is distributed according to a Binomial distribution with number of trials $n$ and probability of success $\lambda / n$. If the intervals are infinitessimally small, in the limit as $n \rightarrow \infty$, then number of trials increases and the Binomial distribution tends to the Poisson distribution:
...
ovarian %>% 
  select(followup_time = futime, censored = fustat)
```
...
$$f(t|\theta) = \int f(t|\theta)p(\theta|y) d\theta$$

And the predictive survival:

$$S(t|\theta) = \int S(t|\theta)p(\theta|y) d\theta$$

These can be calculated analytically for the exponential model:

$$\begin{align}
S(t|\lambda) &= \int_0^\infty e^{-\lambda t}\frac{(\beta + \sum_{i=1}^ny_i)^{\alpha+\sum_{i=1}^n\nu_i}}{\Gamma(\alpha+\sum_{i=1}^n\nu_i)}\lambda^{(\alpha-1+\sum_{i=1}^n\nu_i)}e^{-\lambda(\beta + \sum_{i=1}^ny_i)} d\lambda, \\
&= \frac{(\beta + \sum_{i=1}^ny_i)^{\alpha+\sum_{i=1}^n\nu_i}}{\Gamma(\alpha+\sum_{i=1}^n\nu_i)}\int_0^\infty e^{-\lambda(t + \beta + \sum_{i=1}^ny_i)}\lambda^{(\alpha-1+\sum_{i=1}^n\nu_i)}d\lambda, \\
&= \frac{(\beta + \sum_{i=1}^ny_i)^{\alpha+\sum_{i=1}^n\nu_i}}{\Gamma(\alpha+\sum_{i=1}^n\nu_i)}\frac{\Gamma(\alpha+\sum_{i=1}^n\nu_i)}{(t + \beta + \sum_{i=1}^ny_i)^{\alpha+\sum_{i=1}^n\nu_i}}, \\
&= \left(\frac{\beta + \sum_{i=1}^ny_i}{t + \beta + \sum_{i=1}^ny_i}\right)^{\alpha+\sum_{i=1}^n\nu_i}.
\end{align}$$

Now the survival curve can be plotted for the ovarian data:

```{r}
exponential_survival <- function(alpha, beta, y, nu)
  function(t) {
    nnu <- sum(nu)
    ssy <- sum(y)
    x <- (beta + ssy) / (t + beta + ssy)
    x^(alpha + nnu)
  }

analytic <- ggplot(data = data.frame(x = c(0, 1250)), aes(x)) +
  stat_function(fun = exponential_survival(beta, alpha, ovarian$futime, ovarian$fustat)) +
  ylim(0, 1) +
  xlim(0, 1250)
...
To express this model using `brms`, we model the followup time directly while considering censoring. The event is considered right-censored if a death has not occurred, hence `death = 0` and the formula to specify the followup time is right sensored is `futime | cens(fustat)`. This is followed by distributed by `~` and the covariates of interest. To compare it to the analytic result above we consider the model with only an intercept and no covariates. Note that this model is not exacly the same since the prior is on specified on the log of the rate, $\lambda$.
...
write_rds(x = fit_exponential, path = "~/git/blog/static/models/exponential.Rds")
```

```{r, echo=FALSE}
fit_exponential <-  read_rds("~/git/blog/static/models/exponential.Rds")
```
...
```{r exponential_survival_curve, fig.cap="(Left) Analytic survival function for the exponential model (Right) Sampling based survival function using 100 draws from the posterior distribution"}
