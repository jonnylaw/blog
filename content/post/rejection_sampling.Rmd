...
probability $p(\psi^\star)/Mq(\psi)$, where $M$ is an upper bound on $p/q$. Algorithm~\ref{alg:rejection-sampler} shows a single step of the rejection sampler algorithm which returns a single sample from the target distribution $p(\cdot)$.
