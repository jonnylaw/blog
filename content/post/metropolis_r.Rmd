...
write_rds(out, path = "../../static/models/metropolis_serial.Rds")
```

```{r, echo=FALSE}
out <- read_rds("../../static/models/metropolis_serial.Rds")
```
...
Typically, multiple chains are run in parallel, a straightforward way to do this in R is to use a parallel map from the [furrr](https://davisvaughan.github.io/furrr/) package. 
...
First we create a new function which alters the `metropolis` function to return a dataframe: 
...
write_rds(mh_samples, "../../static/models/metropolis_parallel.Rds")
```

```{r, echo=FALSE}
mh_samples <- read_rds("../../static/models/metropolis_parallel.Rds")
```
...
```{r, eval=FALSE, echo=FALSE}
write_rds(timings, "../../static/models/rcpp.Rds")
```

```{r, echo=FALSE}
timings <- read_rds("../../static/models/rcpp.Rds")
```
