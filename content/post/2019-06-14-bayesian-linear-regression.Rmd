...
date: '2019-06-14'
draft: true
slug: bayesian-model-evaluation
categories: [R, Bayesian]
tags: []
---

TODO: Explore prior distributions
...
This next section is an illustration on how to fit a linear regression model using Gibbs sampling.

Using the likelihood and priors derived in the section above we can derive the conditionally conjugate posterior distributions:
...
It is often useful to transform the data, in this instance we use the function `scale` to centre and scale the data by subtracting the mean and dividing by the standard deviation. This can be recovered

```{r}
ys <- tibble(y = scale(weights), x = scale(heights))
```
...
This can be sped up even more using [RCpp](2019/02/11/efficient_mcmc_rcpp).