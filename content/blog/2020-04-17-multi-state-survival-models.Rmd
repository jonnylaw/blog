...
```{r}
msm::statetable.msm(state = state, subject = replicate, data = replicates)
```
...
qs <- msm::statetable.msm(state = state, subject = replicate, data = replicates)
qs
```
...
$$\begin{aligned}
p(X_{i=13,t_{0:3}}|Q) &= Pr(X_{t_1} = 2|X_{t_0} = 1)Pr(X_{t>t_2} = 3)Pr(X_{t_2} = 1|X_{t_1} = 2)Pr(X_{t>t_2} = 3) \\
...
$$p(X_{i,t_{0:3}}|Q) = \prod_{j=1}^3\prod_{k=1,k\neq j}^3q_{jk}^{n_{jk}}\exp(-\tau_{ij}q_j)$$

Where $q_j = \sum_{j=1,j\neq k}^3q_{jk}$, $\tau_{ij}$ is the time in the $j^{\text{th}}$ state for patient $i$ and $n_{jk}$ is a matrix with a count of the number of transitions from state $j$ to state $k$. We can then calculate the likelihood for all patients:
...
```{r}
observed_transitions <- msm::statetable.msm(state, patient, sims)
...
log_prior <- function(lambda) {
  sum(dgamma(lambda, shape = 3, scale = 3 / 0.1))
}
