...
config <- configr::read.config(here::here("config.toml"))
run_mcmc <- config$runMcmc == 1
write_mcmc <- config$writeMcmc == 1
library(tidyr)
library(dplyr)
library(readr) 
library(ggplot2)
...
```{r metropolis-serial, eval=run_mcmc}
out = metropolis(theta, log_posterior(xs), proposal, 10000)
```

```{r write-iters-serial, eval=write_mcmc}
readr::write_rds(out, path = here::here("models/metropolis-serial-r.Rds"))
```

```{r read-iters-serial, eval=!run_mcmc}
out <- readr::read_rds(here::here("models/metropolis-serial-r.Rds"))
```
...
```{r write-iters, eval=write_mcmc}
readr::write_rds(mh_samples, path = here::here("models/metropolis-r.Rds"))
```

```{r read-iters, eval=!run_mcmc}
mh_samples <- readr::read_rds(here::here("models/metropolis-r.Rds"))
```
...
```{r metropolis-cpp, eval=run_mcmc}
out_cpp <- metropolis(theta, function(p) logDensity(xs, p), function(p) proposalCpp(p, 0.05), 10000)
```

```{r write-iters-cpp, eval=write_mcmc}
readr::write_rds(out_cpp, path = here::here("models/metropolis-cpp.Rds"))
```

```{r read-iters-cpp, eval=!run_mcmc}
out_cpp <- readr::read_rds(here::here("models/metropolis-cpp.Rds"))
```
